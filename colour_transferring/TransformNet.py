...
class Mapping(nn.Module):
    def __init__(self, size):
        ...
        self.size = size
        self.net = nn.Sequential(nn.Linear(self.size, self.size), nn.ReLU())#),nn.Tanh(),nn.Linear(self.size, self.size), nn.Tanh(),nn.Linear(self.size,self.size))

    def forward(self, inputs):
        outputs = self.net(inputs)
        #print(inputs.abs().mean().item(),outputs.abs().mean().item())
...
class TransformNet(nn.Module):
    def __init__(self, size):
        ...
        self.net = nn.Sequential(
            # nn.Linear(self.size,self.size),nn.Tanh(),
            #                      nn.Linear(self.size, self.size), nn.Tanh(),
